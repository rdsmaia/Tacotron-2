...
class TacotronDecoderCell(RNNCell):
	...
	* : This is typically taking a vanilla LSTM, wrapping it using tensorflow's attention wrapper,
	...
	def __init__(self, prenet, speaker_embeddings, attention_mechanism, rnn_cell, frame_projection, stop_projection):
	...
	def __call__(self, inputs, state):
		# NOTE: adding speaker information in the decoder prenet input
#		if self._speaker_embeddings is not None:
#			inputs = tf.concat([inputs, self._speaker_embeddings], axis=-1)

		#Information bottleneck (essential for learning attention)
		prenet_output = self._prenet(inputs)

                # NOTE: adding speaker information in the decoder prenet output
		if self._speaker_embeddings is not None:
			prenet_output = tf.concat([prenet_output, self._speaker_embeddings], axis=-1)
