...
class Tacotron():
	...
	def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, linear_targets=None, targets_lengths=None, gta=False,
			global_step=None, is_training=False, is_evaluating=False, split_infos=None, speaker_id=None, spk_embedding=None):
		"""
		...
		with tf.device(split_device):
			...
			tower_speaker_id = tf.split(speaker_id, num_or_size_splits=hp.tacotron_num_gpus, axis=0) if speaker_id is not None else speaker_id
			tower_spk_embedding = tf.split(spk_embedding, num_or_size_splits=hp.tacotron_num_gpus, axis=0) if spk_embedding is not None else spk_embedding
		...
		for i in range(hp.tacotron_num_gpus):
			with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
				with tf.variable_scope('inference') as scope:
					...
					if tower_speaker_id is not None:
						#Speaker embedding
						if hp.use_dvectors is False:
						...
						if (is_training or is_evaluating) and hp.spk_dependent_embedding:
							speaker_embeddings = tower_spk_embedding[i]
						else:
							speaker_embeddings = tf.nn.embedding_lookup(self.speaker_embedding_table, tower_speaker_id[i])

						#Repeat speaker info along time axis
						if hp.speaker_embeddings_encoder:
							tiled_speaker_embeddings = tf.tile(tf.expand_dims(speaker_embeddings, axis=1), [1, tf.shape(encoder_outputs)[1], 1])

							#Concat speaker info to the encoder outputs
							encoder_outputs = tf.concat([encoder_outputs, tiled_speaker_embeddings], axis=-1)
					...
					if hp.speaker_embeddings_decoder:
					...
					if hp.speaker_embeddings_postnet:
						# add speaker embeddings (to be used for future unsupervised adaptation)
						speaker_embeddings_post = tf.layers.dense(speaker_embeddings, hp.num_mels, use_bias=False)
						tiled_speaker_embeddings_post = tf.tile(tf.expand_dims(speaker_embeddings_post, axis=1), [1, tf.shape(decoder_output)[1], 1])

						#Compute residual using post-net ==> [batch_size, decoder_steps * r, postnet_channels]
						residual = postnet(decoder_output + tiled_speaker_embeddings_post)
					else:
						#Compute residual using post-net ==> [batch_size, decoder_steps * r, postnet_channels]
			...
			with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
				with tf.variable_scope('loss') as scope:
					if hp.mask_decoder:
						...
						#before = tf.losses.absolute_difference(self.tower_mel_targets[i], self.tower_decoder_output[i])
						# Compute loss after postnet
					...
					regularization = tf.add_n([tf.nn.l2_loss(v) for v in self.all_vars if not('bias' in v.name or 'Bias' in v.name or '_projection' in v.name or 'inputs_embedding' in v.name or 'speaker_embeddings' in v.name or 'RNN' in v.name or 'LSTM' in v.name)]) * reg_weight
