...
class Synthesizer:
	def load(self, checkpoint_path, hparams, gta=False, speaker_id=None, model_name='Tacotron'):
		...
		with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
			...
			if gta:
				self.model.initialize(inputs, input_lengths, targets, gta=gta, split_infos=split_infos, speaker_id=speaker_id)
			else:
				self.model.initialize(inputs, input_lengths, split_infos=split_infos, speaker_id=speaker_id)
	...
	def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, speaker_id=None):
		...
		#if speaker_id is not None:
		feed_dict[self.speaker_id] = np.asarray([str(speaker_id)], dtype=np.int32)
#		feed_dict[self.speaker_id] = np.asarray([str(s) for s in speaker_id], dtype=np.int32)
		...
		if self.gta or not hparams.predict_linear:
			...
			for k, align in enumerate(alignments):
				print(f'Aligment shape: (phones, timesteps) = {align.shape}')
				print(f'Output spectrogram shape: {mels[k].shape}')
		...
		for i, mel in enumerate(mels):
			...
			#	ori_speaker_id = '<no_g>' #set the rule to determine speaker id. By using the file basename maybe? (basenames are inside "basenames" variable)
			#	speaker_ids.append(ori_speaker_id) #finish by appending the speaker id. (allows for different speakers per batch if your model is multispeaker)
			#else:
			ori_speaker_id = '<no_g>'
			speaker_ids.append(ori_speaker_id)

			# Write the spectrogram to disk
			# Note: outputs mel-spectrogram files and target ones have same names, just different folders
			if self.gta is False:
				mel_filename = os.path.join(out_dir, 'mel-{}.npy'.format(basenames[i]+'_speaker_'+str(speaker_id)))
			else:
			...
			if log_dir is not None:
				...
				audio.save_wav(wav, os.path.join(log_dir, 'wavs/wav-{}-mel.wav'.format(basenames[i]+'_speaker_'+str(speaker_id))), sr=hparams.sample_rate)

				#save alignments
				plot.plot_alignment(alignments[i], os.path.join(log_dir, 'plots/alignment-{}.png'.format(basenames[i]+'_speaker_'+str(speaker_id))),
					title='{}'.format(texts[i]), split_title=True, max_len=target_lengths[i])

				#save mel spectrogram plot
				plot.plot_spectrogram(mel, os.path.join(log_dir, 'plots/mel-{}.png'.format(basenames[i]+'_speaker_'+str(speaker_id))),
